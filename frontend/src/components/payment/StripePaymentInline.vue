          ...
          <div class="w-full rounded-xl bg-gray-50 p-4 dark:bg-dark-800">
            <div class="space-y-2 text-sm">
              ...
              <div v-if="amount > 0" class="flex justify-between">
                ...
                <span class="font-medium text-gray-900 dark:text-white">{{ orderType === 'balance' ? '$' : 'RM' }}{{ amount.toFixed(2) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</span>
                <span class="font-medium text-gray-900 dark:text-white">RM{{ payAmount.toFixed(2) }}</span>
              </div>
            </div>
          </div>
    ...
    <template v-else>
      ...
      <div class="card overflow-hidden">
        <div class="bg-gradient-to-br from-[#635bff] to-[#4f46e5] px-6 py-5 text-center">
          ...
          <p class="mt-1 text-3xl font-bold text-white">RM{{ payAmount.toFixed(2) }}</p>
        </div>
      </div>
