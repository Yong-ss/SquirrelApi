    ...
    <div v-if="order" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p class="text-sm font-medium text-gray-900 dark:text-white">RM{{ baseAmount.toFixed(2) }}</p>
        </div>
        <div v-if="order.fee_rate > 0">
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.fee') }} ({{ order.fee_rate }}%)</p>
          <p class="text-sm font-medium text-gray-900 dark:text-white">RM{{ feeAmount.toFixed(2) }}</p>
        </div>
        <div>
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</p>
          <p class="text-sm font-medium text-gray-900 dark:text-white">RM{{ order.pay_amount.toFixed(2) }}</p>
        </div>
        <div v-if="order.amount !== order.pay_amount">
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.creditedAmount') }}</p>
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ order.order_type === 'balance' ? '$' : 'RM' }}{{ order.amount.toFixed(2) }}</p>
        </div>
        <div>
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-2 text-sm">
          <div>
            ...
            <span class="ml-1 font-medium text-red-700 dark:text-red-300">{{ order.order_type === 'balance' ? '$' : 'RM' }}{{ order.refund_amount.toFixed(2) }}</span>
          </div>
