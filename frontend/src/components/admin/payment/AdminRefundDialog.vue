    ...
    <form id="refund-form" @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div class="rounded-lg bg-gray-50 p-3 dark:bg-dark-700">
        ...
        <div class="mt-1 flex justify-between text-sm">
          ...
          <span class="font-medium text-gray-900 dark:text-white">{{ order?.order_type === 'balance' ? '$' : 'RM' }}{{ order?.amount?.toFixed(2) }}</span>
        </div>
        <div class="mt-1 flex justify-between text-sm">
          <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</span>
          <span class="font-medium text-gray-900 dark:text-white">RM{{ order?.pay_amount?.toFixed(2) }}</span>
        </div>
        <div v-if="actuallyRefunded > 0" class="mt-1 flex justify-between text-sm">
          ...
          <span class="font-medium text-red-600 dark:text-red-400">{{ order?.order_type === 'balance' ? '$' : 'RM' }}{{ actuallyRefunded.toFixed(2) }}</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div v-if="form.deduct_balance && userBalance != null" class="mt-3 grid grid-cols-2 gap-3">
          ...
          <div class="rounded-lg bg-gray-50 p-3 text-sm dark:bg-dark-700">
            ...
            <div class="mt-1 font-semibold text-gray-900 dark:text-white">{{ order?.order_type === 'balance' ? '$' : 'RM' }}{{ order?.amount?.toFixed(2) }}</div>
          </div>
        </div>
      ...
      <div>
        ...
        <div class="relative">
          <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{{ order?.order_type === 'balance' ? '$' : 'RM' }}</span>
          <input
        ...
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          {{ t('payment.admin.maxRefundable') }}: {{ order?.order_type === 'balance' ? '$' : 'RM' }}{{ maxRefundable.toFixed(2) }}
        </p>
      </div>
