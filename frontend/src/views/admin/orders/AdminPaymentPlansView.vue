    ...
    <div class="space-y-4">
      ...
      <DataTable :columns="planColumns" :data="plans" :loading="plansLoading">
        ...
        <template #cell-price="{ value, row }">
          <div class="text-sm">
            <span class="font-medium text-gray-900 dark:text-white">RM{{ (value ?? 0).toFixed(2) }}</span>
            <span v-if="row.original_price" class="ml-1 text-xs text-gray-400 line-through">RM{{ row.original_price.toFixed(2) }}</span>
          </div>
        </template>
